...
public class TarInputStream : Stream
{
    ...
    private async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken ct, bool isAsync)
    {
        ...
        if (readBuffer != null)
        {
            var sz = (numToRead > readBuffer.Memory.Length) ? readBuffer.Memory.Length : (int)numToRead;
            ...
            {
                var newLen = readBuffer.Memory.Length - sz;
    ...
    private async ValueTask SkipAsync(long skipCount, CancellationToken ct, bool isAsync)
    {
        ...
        {
            var toRead = num > length ? length : (int)num;
            var numRead = await ReadAsync(skipBuf.Memory.Slice(0, toRead), ct, isAsync).ConfigureAwait(false);
    ...
    private async ValueTask<TarEntry> GetNextEntryAsync(CancellationToken ct, bool isAsync)
    {
        ...
        var headerBuf = ArrayPool<byte>.Shared.Rent(TarBuffer.BlockSize);
            ...
            try
            {
                ...
                if (header.TypeFlag == TarHeader.LF_GNU_LONGNAME)
                {
                    ...
                    while (numToRead > 0)
                    {
                        ...
                        var numRead = await ReadAsync(nameBuffer.Memory.Slice(0, length), ct, isAsync).ConfigureAwait(false);
                ...
                else if (header.TypeFlag == TarHeader.LF_XHDR)
                {
                    ...
                    var nameBuffer = ArrayPool<byte>.Shared.Rent(TarBuffer.BlockSize);
                    var numToRead = this.entrySize;
                    ...
                    while (numToRead > 0)
                    {
                        ...
                        var numRead = await ReadAsync(nameBuffer.AsMemory().Slice(0, length), ct, isAsync).ConfigureAwait(false);
    ...
    private async ValueTask CopyEntryContentsAsync(Stream outputStream, CancellationToken ct, bool isAsync)
    {
        ...
        while (true)
        {
            var numRead = await ReadAsync(tempBuffer, ct, isAsync).ConfigureAwait(false);
            if (numRead <= 0)
            {
                break;
            }
