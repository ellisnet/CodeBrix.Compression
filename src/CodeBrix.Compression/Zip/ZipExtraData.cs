...
public class NTTaggedData : ITaggedData
{
    ...
    public void SetData(byte[] data, int index, int count)
    {
        ...
        {
            var ntfsTag = ms.ReadLEShort();
            var ntfsLength = ms.ReadLEShort();
            if (ntfsTag == 1)
            {
                if (ntfsLength >= 24)
                {
                    var lastModificationTicks = ms.ReadLELong();
                    _lastModificationTime = DateTime.FromFileTimeUtc(lastModificationTicks);

                    var lastAccessTicks = ms.ReadLELong();
                    _lastAccessTime = DateTime.FromFileTimeUtc(lastAccessTicks);

                    var createTimeTicks = ms.ReadLELong();
