...
namespace CodeBrix.Compression.Encryption;
...
internal class ZipAESTransform : ICryptoTransform
{
    ...
    public ZipAESTransform(string key, byte[] saltBytes, int blockSize, bool writeMode)
    {
        ...
        var keyBytes = Rfc2898DeriveBytes.Pbkdf2(key, saltBytes, KEY_ROUNDS, HashAlgorithmName.SHA1,
            _blockSize + _blockSize + PWD_VER_LENGTH);
        var rm = Aes.Create();
        rm.Mode = CipherMode.ECB;           // No feedback from cipher for CTR mode
        _counterNonce = new byte[_blockSize];
        var key1bytes = keyBytes.AsSpan(0, _blockSize);
        var key2bytes = keyBytes.AsSpan(_blockSize, _blockSize);

        // Use empty IV for AES
        _encryptor = rm.CreateEncryptor(key1bytes.ToArray(), new byte[16]);
        _pwdVerifier = keyBytes.AsSpan(_blockSize + _blockSize, PWD_VER_LENGTH).ToArray();
        //
    ...
    public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
    {
        ...
        if (inputCount != 0) {
            if (inputCount > ZipAESStream.AUTH_CODE_LENGTH)
            {
                ...
                var finalBlock = inputCount - ZipAESStream.AUTH_CODE_LENGTH;
