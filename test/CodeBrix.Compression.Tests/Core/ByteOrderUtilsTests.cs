...
public class ByteOrderUtilsTests
{
    ...
    {
        ClassicAssert.AreEqual(swapped16, BO.SwappedBytes(native16));
        ClassicAssert.AreEqual(swapped16, BO.SwappedBytes((ushort)native16));

        ClassicAssert.AreEqual(swapped32, BO.SwappedBytes(native32));
        ClassicAssert.AreEqual(swapped32, BO.SwappedBytes((uint)native32));

        ClassicAssert.AreEqual(swapped64, BO.SwappedBytes(native64));
        ClassicAssert.AreEqual(swapped64, BO.SwappedBytes((ulong)native64));
    }

    [Test]
    public void FromSwappedBytes()
    {
        ClassicAssert.AreEqual(native16, BO.SwappedS16(swapped16));
        ClassicAssert.AreEqual(native16, BO.SwappedU16(swapped16));
			
        ClassicAssert.AreEqual(native32, BO.SwappedS32(swapped32));
        ClassicAssert.AreEqual(native32, BO.SwappedU32(swapped32));
			
        ClassicAssert.AreEqual(native64, BO.SwappedS64(swapped64));
        ClassicAssert.AreEqual(native64, BO.SwappedU64(swapped64));
    }
    ...
    private static void TestReadLE<T>(T expected, int bytes, Func<Stream, T> read)
    {
        ...
        ClassicAssert.AreEqual(expected, read(ms));
    }
    ...
    private static void TestWriteLE(byte[] expected, Action<Stream> write)
    {
        ...
        write(ms);
        ClassicAssert.AreEqual(expected, ms.ToArray());
    }
