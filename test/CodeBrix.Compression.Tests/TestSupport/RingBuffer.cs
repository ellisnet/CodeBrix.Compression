...
public class ExerciseBuffer
{
    ...
    public void Basic()
    {
        ...
        ClassicAssert.IsFalse(buffer_.IsFull);
        ClassicAssert.IsTrue(buffer_.IsEmpty);

        buffer_.WriteByte(1);

        ClassicAssert.IsFalse(buffer_.IsFull);
        ClassicAssert.IsFalse(buffer_.IsEmpty);
        ClassicAssert.AreEqual(1, buffer_.Count);

        ClassicAssert.AreEqual(1, buffer_.ReadByte());
        ...
        ClassicAssert.IsTrue(buffer_.IsFull);
        ClassicAssert.IsFalse(buffer_.IsEmpty);

        buffer_.Close();

        ClassicAssert.IsTrue(buffer_.IsClosed);
        ...
        while (count != 0)
        {
            ClassicAssert.AreEqual(count, buffer_.Count);
            ClassicAssert.AreEqual(expected, buffer_.ReadByte());
            count--;
    ...
    public void Buffered()
    {
        ...
        ClassicAssert.AreEqual(3, buffer_.Count);

        byte[] readBuffer = new byte[16];
        ClassicAssert.AreEqual(3, buffer_.Read(readBuffer, 0, 3));
        for (int i = 0; i < 3; ++i)
        {
            ClassicAssert.AreEqual(i, readBuffer[i]);
        }
    }
    ...
    private void Reader()
    {
        ...
        while (readTarget_ > 0)
        {
            ...
            while (thisTime > 0)
            {
                ...
                ClassicAssert.AreEqual(nextValue, readValue);
